...
def old_chat(chat_rooms: ChatRooms):
    ...
    for r in records:
        prefix = f'\n{customize_string("User", CustomizeOption.GREEN)}' if r['role'] == 'user' else \
            f'{customize_string("GPT", CustomizeOption.BLUE)} '
        print(f'{prefix} > {r["content"]}')
    size = get_size(gpt.path)
    if size > 102400:
        print(f'\n{customize_string("Tip", CustomizeOption.YELLOW)} > 当前聊天记录过多，建议重新开始新对话')
    chat(gpt)
